...
function setupFixtures() {
  ...
  // Create test files
  fs.writeFileSync(
    path.join(FIXTURE_DIR, "widget.ts"),
    'export const widget = "test"',
  )
  fs.writeFileSync(
    path.join(FIXTURE_DIR, "widget-loader.ts"),
    'import { widget } from "./widget"',
  )
  fs.writeFileSync(
    path.join(FIXTURE_DIR, "WidgetConfig.ts"),
    "export interface WidgetConfig {}",
  )
  fs.mkdirSync(path.join(FIXTURE_DIR, "testing"), { recursive: true })
  fs.writeFileSync(
    path.join(FIXTURE_DIR, "testing/fake-widget.ts"),
    "export class FakeWidget {}",
  )

  // Create a file that would conflict (gadget.ts already exists)
  fs.writeFileSync(
    path.join(FIXTURE_DIR, "gadget.ts"),
    'export const gadget = "existing"',
  )
}
...
describe("applyReplacement", () => {
  test("replaces lowercase", () => {
    expect(applyReplacement("widget-loader.ts", "widget", "gadget")).toBe(
      "gadget-loader.ts",
    )
  })

  test("preserves PascalCase", () => {
    expect(applyReplacement("WidgetConfig.ts", "widget", "gadget")).toBe(
      "GadgetConfig.ts",
    )
  })

  test("preserves UPPERCASE", () => {
    expect(applyReplacement("WIDGET_ROOT.ts", "widget", "gadget")).toBe(
      "GADGET_ROOT.ts",
    )
  })

  test("handles multiple occurrences", () => {
    expect(applyReplacement("widget-widget.ts", "widget", "gadget")).toBe(
      "gadget-gadget.ts",
    )
  })

  test("handles mixed case in same file", () => {
    expect(applyReplacement("WidgetLoader-widget.ts", "widget", "gadget")).toBe(
      "GadgetLoader-gadget.ts",
    )
  })
})
...
describe("read-only file operations", () => {
  ...
  describe("findFilesToRename", () => {
    test("finds files matching pattern", async () => {
      const ops = await findFilesToRename(
        "widget",
        "gadget",
        "**/*.ts",
        FIXTURE_DIR,
  ...
  describe("checkFileConflicts", () => {
    test("detects target exists conflict", async () => {
      ...
      const widgetConflict = report.conflicts.find(
        (c) => c.oldPath === "widget.ts",
      )
    ...
    test("identifies safe renames", async () => {
      ...
      const loaderOp = report.safe.find(
        (op) => op.oldPath === "widget-loader.ts",
      )
  ...
  describe("createFileRenameProposal", () => {
    test("creates editset with file ops", async () => {
      const editset = await createFileRenameProposal(
        "widget",
        "gadget",
        "**/*.ts",
        FIXTURE_DIR,
...
describe("verifyFileEditset mutations", () => {
  ...
  test("detects file changes", async () => {
    ...
    expect(result.drifted.some((d) => d.includes("widget-loader.ts"))).toBe(
      true,
    )
  })
})
...
describe("applyFileRenames", () => {
  ...
  test("dry run does not rename files", async () => {
    ...
    expect(fs.existsSync(path.join(FIXTURE_DIR, "gadget-loader.ts"))).toBe(
      false,
    )
  ...
  test("applies renames", async () => {
    ...
    expect(fs.existsSync(path.join(FIXTURE_DIR, "widget-loader.ts"))).toBe(
      false,
    )
    ...
    expect(
      fs.existsSync(path.join(FIXTURE_DIR, "testing/fake-gadget.ts")),
    ).toBe(true)
  })
